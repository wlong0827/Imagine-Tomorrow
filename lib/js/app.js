...
;(function (h, j, e) {
  var a = "placeholder" in j.createElement("input");var f = "placeholder" in j.createElement("textarea");var k = e.fn;var d = e.valHooks;var b = e.propHooks;var m;var l;if (a && f) {
    l = k.placeholder = function () {
      return this;
    };l.input = l.textarea = true;
  } else {
    l = k.placeholder = function () {
      var n = this;n.filter((a ? "textarea" : ":input") + "[placeholder]").not(".placeholder").bind({ "focus.placeholder": c, "blur.placeholder": g }).data("placeholder-enabled", true).trigger("blur.placeholder");return n;
    };l.input = a;l.textarea = f;m = { get: function get(o) {
        var n = e(o);var p = n.data("placeholder-password");if (p) {
          return p[0].value;
        }return n.data("placeholder-enabled") && n.hasClass("placeholder") ? "" : o.value;
      }, set: function set(o, q) {
        var n = e(o);var p = n.data("placeholder-password");if (p) {
          return p[0].value = q;
        }if (!n.data("placeholder-enabled")) {
          return o.value = q;
        }if (q == "") {
          o.value = q;if (o != j.activeElement) {
            g.call(o);
          }
        } else {
      ...
      d.textarea = m;b.value = m;
    }e(function () {
      e(j).delegate("form", "submit.placeholder", function () {
        var n = e(".placeholder", this).each(c);setTimeout(function () {
          n.each(g);
        }, 10);
      });
    ...
    var n = {};var p = /^jQuery\d+$/;e.each(o.attributes, function (r, q) {
      if (q.specified && !p.test(q.name)) {
        n[q.name] = q.value;
      }
    });return n;
  }function c(o, p) {
    var n = this;var q = e(n);if (n.value == q.attr("placeholder") && q.hasClass("placeholder")) {
      if (q.data("placeholder-password")) {
        q = q.hide().next().show().attr("id", q.removeAttr("id").data("placeholder-id"));if (o === true) {
          return q[0].value = p;
        }q.focus();
      } else {
        n.value = "";q.removeClass("placeholder");n == j.activeElement && n.select();
      }
    }
  }function g() {
    var r;var n = this;var q = e(n);var p = this.id;if (n.value == "") {
      if (n.type == "password") {
        if (!q.data("placeholder-textinput")) {
          ...
          }r.removeAttr("name").data({ "placeholder-password": q, "placeholder-id": p }).bind("focus.placeholder", c);q.data({ "placeholder-textinput": r, "placeholder-id": p }).before(r);
        }q = q.removeAttr("id").hide().prev().attr("id", p).show();
      }q.addClass("placeholder");q[0].value = q.attr("placeholder");
    } else {
...
+function ($) {
  ...
  var Bjax = function Bjax(element, options) {
  ...
  $.fn.bjax = function (option) {
    return this.each(function () {
      ...
      var options = $.extend({}, Bjax.DEFAULTS, $this.data(), (typeof option === "undefined" ? "undefined" : _typeof(option)) == 'object' && option);
