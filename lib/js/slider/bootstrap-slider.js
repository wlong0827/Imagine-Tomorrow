...
(function ($) {
	...
	var Slider = function Slider(element, options) {
		...
		var self = this;
		$.each(['min', 'max', 'step', 'precision', 'value', 'reversed', 'handle'], function (i, attr) {
	...
	Slider.prototype = {
		...
		layout: function layout() {
			...
			if (this.range) {
				this.tooltipInner.text(this.formater(this.value[0]) + this.tooltip_separator + this.formater(this.value[1]));
		...
		triggerFocusOnHandle: function triggerFocusOnHandle(handleIdx) {
			if (handleIdx === 0) {
				this.handle1.focus();
			}
			if (handleIdx === 1) {
		...
		keydown: function keydown(handleIdx, ev) {
			if (!this.isEnabled()) {
				return false;
			}
		...
		adjustPercentageForRangeSliders: function adjustPercentageForRangeSliders(percentage) {
			if (this.range) {
		...
		applyPrecision: function applyPrecision(val) {
  ...
  */
		getNumDigitsAfterDecimalPlace: function getNumDigitsAfterDecimalPlace(num) {
		...
		applyToFixedAndParseFloat: function applyToFixedAndParseFloat(num, toFixedInput) {
		...
		validateInputValue: function validateInputValue(val) {
	...
	function createNewSliderInstance(opts) {
		...
		$this.each(function () {
			var $this = $(this),
			    ...
			    options = (typeof opts === "undefined" ? "undefined" : _typeof(opts)) === 'object' && opts;
